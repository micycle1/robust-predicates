 ...
 * (port of the rule chain in {@code forward_error_bound.hpp} using the root
 * finalization recipe of {@code stage_a_error_bound.hpp}).
 *
 ...
 * an eps-polynomial coefficient triple. Rules are tried in the fixed order of
 * the C++ {@code robust_rules} list (inputs assumed exact):
 * <ol>
 ...
 * </ol>
 *
 * <p>With underflow protection enabled (the default, matching
 * {@code robust_rules<true>}), rules whose absolute error can underflow append
...
public final class ErrorBoundDeriver {
    ...
    /** Deriver with underflow protection, matching C++ {@code robust_rules<true>}. */
     ...
     * exact (C++ {@code pow2_product::pow2_exp}).
     */
     ...
     * trailing guard (C++ {@code collapse_underflow_guards}).
     */
     ...
     * (C++ {@code forward_error_condition_sumdiff}).
     */
     ...
     * (C++ {@code forward_error_bound_expression}).
     */
