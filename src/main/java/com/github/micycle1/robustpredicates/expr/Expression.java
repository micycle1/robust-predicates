 ...
 *
 * <p>This is the Java analogue of the type-level expression trees in the C++
 * framework ({@code expression_tree.hpp}): where the C++ version encodes an
 * expression such as {@code (_3 - _1) * (_6 - _2)} in the type system and lets
 * the compiler deduplicate identical subexpressions via {@code std::is_same},
 * this class interns every node in a global pool so that structural equality
 ...
 * <p>Node kinds:
 * <ul>
 *   <li>{@link Op#ARGUMENT}: a 1-based input placeholder (like the C++
 *       {@code _1}..{@code _20}), see {@link #argN()};</li>
 *   <li>{@link Op#CONSTANT}: a fixed value; an <em>underflow guard</em> is a
 *       constant with {@link #guardCount()} {@code > 0} and value
 *       {@code guardCount * Double.MIN_NORMAL} (the analogue of the C++
 *       {@code underflow_guard_constant<CT, c>});</li>
...
public final class Expression {
    ...
    /** Input placeholder with 1-based index (the C++ {@code _1}, {@code _2}, ...). */
     ...
     * {@code count * Double.MIN_NORMAL} (C++ {@code underflow_guard_constant<CT, c>}).
     */
    ...
    /** 1-based argument index; 0 for non-argument nodes (mirrors C++ {@code argn}). */
    public int argN() {
        return argN;
    }
    ...
    /** Statically-known non-negativity (mirrors C++ {@code non_negative}). */
