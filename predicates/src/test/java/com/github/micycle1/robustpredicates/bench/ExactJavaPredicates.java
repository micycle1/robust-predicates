...
/**
 * Shewchuk-style adaptive exact geometric predicates, copied from ProGAL's
 * {@code ProGAL.geom3d.predicates.ExactJavaPredicates}
 * (https://github.com/Bio7/ProGAL, {@code src/ProGAL/geom3d/predicates/ExactJavaPredicates.java}).
 *
 * <p>Only the self-contained numeric core is retained: the ProGAL
 * {@code Point}/{@code Triangle}/{@code Tetrahedron}-based wrapper methods and
 * the {@code extends Predicates} base class were stripped so the file compiles
 * with no ProGAL dependency. The double[]-based {@code orient2d},
 * {@code orient3d}, {@code incircle} and {@code insphere} routines are byte-for-
 * byte identical to the original and are made {@code public} here so the JMH
 * benchmarks can call them. Used only as a benchmark baseline.
 */
public class ExactJavaPredicates {
	...
	@SuppressWarnings("unused")
	public double orient2d(double[] pa,double[] pb,double[] pc)
	{
	...
	public double orient3d(double[] pa,double[]  pb,double[]  pc,double[]  pd)
	{
		double adx, bdx, cdx, ady, bdy, cdy, adz, bdz, cdz;
...
//			System.out.println("Returning early "+det+" .. "+errbound);
	...
	public double insphere(double[] pa,double[]  pb,double[]  pc,double[]  pd,double[]  pe)
	{
		double aex, bex, cex, dex;
		double aey, bey, cey, dey;
		double aez, bez, cez, dez;
